---
title: "Hyptohesis Test for single proportion with CLT"
...
-   Project proposals due Wednesday (start working in class)
...
If you'd like supplemental reading about CLT-based inference for one proportion, [Section 16.2](https://openintro-ims.netlify.app/inference-one-prop#sec-one-prop-norm) of the textbook can be helpful!
...
If 1) $H_{0}$ true and 2) success-failure (under $H_{0}$) and independence conditions are met, then CLT tells us our **null distribution** is:
...
-   Will obtain p-value using a **test statistic**: a quantity that assesses how consistent our sampled data are with $H_{0}$
    ...
    -   For this specific test, our test statistic is the $z$-score of $\hat{p}_{obs}$ from the null distribution:
...
::: discuss
Remember, $z$ here is our test statistic. Is large $|z|$ or small $|z|$ considered unusual for $H_{0}$?
:::
...
-   p-value is then $\text{Pr}(Z \geq z)$ or $\text{Pr}(Z \leq z)$ (or both), depending on $H_{A}$
    ...
    -   Easily obtained using `pnorm()` : either `pnorm(z, 0, 1)` and/or `1 - pnorm(z, 0, 1)`
...
3.  **Obtain null distribution and p-value**
    ...
    i.  ::: discuss
        ...
        -   Success-failure (under $H_{0}$): $np_{0} = 80(0.5) = 40 \geq 10$ and $n(1-p_{0}) = 40 \geq 10$
