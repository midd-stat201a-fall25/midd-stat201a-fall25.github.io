...
-   $H_{A}$: the variables `sex` and `decision` are *not* independent; equally-qualified female personnel are less likely to be promoted than male personnel
...
Research question: For patients who undergo CPR after a heart attack, does the blood thinner treatment have an effect on survival?

-   $H_{0}:$ the blood thinner treatment has no effect on survival after heart attack 

-   $H_{A}:$ the blood thinner treatment has an effect on survival after heart attack 
...
-   Is this "convincing evidence" that blood thinner usage after CPR has an effect on survival?
...
diff_prop_null <- data.frame(diffs = diff_props_null) |>
  mutate(extreme = (diffs >= obs_diff | diffs <= -obs_diff)) 
diff_prop_null |>
  ggplot(aes(x = diffs))+
...
::: discuss
How would we obtain the p-value in this problem? What does it mean to be "as or more extreme" in the direction of $H_{A}$?
:::
...
diff_prop_null |>
  ...
  geom_histogram(binwidth = 0.025, col = "white", alpha = 0.7)+
  labs(x = "Difference in survival rates (treat - control)",
       title = "Null distribution") +
  ...
  geom_vline(xintercept = p_hat_c - p_hat_t , linetype = "dashed", linewidth = 2, col = "magenta")+
    guides(fill = "none") 
...
The researchers are interested in learning if the blood thinner treatment has an effect on survival after heart attack.
...
-   **Conclusion**: the data do not provide convincing evidence that the blood thinner treatment affects survival rates among patients who undergo CPR.
