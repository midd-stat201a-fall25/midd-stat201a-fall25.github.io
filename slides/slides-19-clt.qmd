...
2.  Normality condition:
    ...
    -   If the data $x_{1},\ldots, x_{n}$ are known to be Normal and independent, then the distribution of $\bar{X}$ is ***exactly*** $N(\mu, \frac{\sigma}{\sqrt{n}})$
...
-   Suppose that the average service time for all customers is 5 minutes, with a standard deviation of 6 minutes. The bank will not allow any customer to wait an absurdly long amount of time.

::: discuss
-   Assume that a bank currently has 36 customers in it, and all customers' wait times are independent of each other. We want to find the probability that the average service time of all these customers is less than 4 minutes.
    -   Write down probability of interest as $\text{Pr}()$ statement
...
-   Conditions for CLT met: independence (given) and sufficiently large sample size $(n=36)$.
...
-   Formula for a (symmetric) $\gamma \times 100\%$ confidence interval:
    ...
    $$
    \text{point estimate} \pm \color{blue}{\text{critical value} \times \text{SE}} \equiv \text{point estimate} \pm \color{blue}{\text{Margin of Error}}
    $$
