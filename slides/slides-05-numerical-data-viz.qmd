...
-   `anscombe` data frame: four datasets each with 11 observations each and the same two variables x and y
...
-   We have learned how to use base `R` code to make simple histograms. We will now use `ggplot2` which makes customization easier
...
-   Let's visualize the four data sets. What would be an appropriate type of plot to examine the relationship between the two quantitative variables `x` and `y`?
...
-   We will learn how to create histograms, box plots, and scatterplots using the `ggplot()` function from the `ggplot2` library (included in `tidyverse`)
