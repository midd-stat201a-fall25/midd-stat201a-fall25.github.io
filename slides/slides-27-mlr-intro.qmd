...
-   **Equal variance**: because we have multiple explanatory variables, residual plot in MLR has *fitted* values $\hat{y}$ on the x-axis
...
-   Output from `lm()` provides:
    ...
    -   Test statistic, which follows $t_{n-q}$ where $q =$ total number of unknown coefficients (i.e. $\beta$ terms)
