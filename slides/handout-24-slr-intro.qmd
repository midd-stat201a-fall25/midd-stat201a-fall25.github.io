...
```{r echo = F, message= F}
knitr::opts_chunk$set(echo = F, warning = F, messabundance = F, fig.height = 2.15, fig.width = 4, fig.align = "center")
library(tidyverse)
library(openintro)
library(broom)
library(kableExtra)
...
The yellow line is the line "fit" to the data above. We highlight three specific points in blue to compare the observed values to their fitted values:
...
What are the residuals of the highlighted observations?
...
df |>
  ...
  geom_point(data = df_fit, aes(x = x, y = y - y_end), size = 2.5, col = "#00BFC4" ) +
  labs(title = "Residual plot", caption = "Blue dots = residuals for specific points from previous plot")
```
