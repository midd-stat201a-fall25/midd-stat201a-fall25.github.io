...
3.  The probability of getting in a car accident is the same if using a cell phone than if not using a cell phone.
...
1.  New York is known as "the city that never sleeps''. A random sample of 25 New Yorkers were asked how much they sleep per night. Do these data providing convincing evidence that New Yorkers on average sleep less than 8 hours per night?
...
-   ::: discuss
    ...
    -   Define $p$ as the true proportion of Middlebury students who pronounce as Middle-"burry".
    -   $H_{0}:\ p = 0.5$ versus $H_{A}:\ p < 0.5$
    -   **Note**: how you define $p$ is important!

## Step 2: Collect and summarize data

```{r echo = F}
class_dat <- read_csv("../data/stat201_data.csv")
x <- rep(1, nrow(class_dat))
x[class_dat$pronounciation == "berry"] <- 0
n <- length(x)
p_hat <- mean(x)
```

Our sample is the convenience sample I took of our class: `r x`, where 1 = "burry" and 0 = "berry".

-   Point estimate: $\hat{p}_{obs} = `r round(p_hat,3)`$
...
-   ::: important
    We calculate a [**p-value**]{.underline}: the probability of observing data as or more extreme than we did, *assuming* $H_{0}$ true
    :::
...
::: discuss
-   **To simulate under** $H_{0}$, what proportion of the slips in the bag should be orange vs blue?
:::

-   To simulate under $H_{0}: p = 0.50$, half of the slips should be orange!
...
null_props <- rep(NA, B)
for(b in 1:B){
  null_samp <- sample(x = c("orange", "blue"), 
                      size = n,
                      replace = T,
                      prob = c(0.5, 0.5)) 
  null_props[b] <- sum(null_samp == "orange")/n
}
```
:::
...
null_dist <- data.frame(p_null = null_props) |>
  ...
  labs(x = "Sample proportion \'burry\'", title = "Null distribution") +
...
-   Our observed point estimate was $\hat{p}_{obs} =$ `r round(p_hat, 3)`
...
```{r fig.height=8}
null_dist +
  labs(title = "Null distribution",caption = "orange line = observed sample proportion") +
  geom_vline(xintercept = p_hat, col = "orange", linewidth = 2) +
...
data.frame(p_null = null_props) |>
  ...
  scale_x_continuous(breaks = seq(0,1,0.1)) +
  labs(x = "Sample proportion \`burry\'") +
  theme(text = element_text(size = 25)) +
  labs(title = "Null distribution") +
  geom_vline(xintercept = p_hat, col = "orange", size = 2) +
...
-   p-value is $\frac{ `r sum(null_props <= p_hat)`}{`r B`} \approx$ `r round(p_val, 3)`
...
1.  Interpret the p-value `r p_val` in context
    ...
    -   ::: discuss
        Assuming $H_{0}$ true, the probability of observing a sample proportion of students saying "burry" as or more extreme as `r p_hat` is approximately `r round(p_val, 3)`
        :::
...
-   We have some control in this - how? Through $\alpha$!
    ...
    -   Lower $\alpha$ means lower chance of Type I error, but comes at cost of increased chance of Type II error
...
-   Do you understand what a p-value represents, and how we obtain it from the null distribution?
    ...
    -   Is the p-value a marginal, joint, or conditional probability?
