...
-   When explanatory $x$ is categorical, we have a slightly more nuanced interpretation
    -   Created indicator variable with 0 = base level and 1 = non-base level
-   Coefficient of determination $R^2$ assesses strength of linear model fit

## Live code!

How to fit a sample linear regression model in `R`. (Code posted after class!)

# Inference for SLR

## Variability of coefficient estimates

*No need to take notes on this slide!*
...
n <- nrow(evals)
df <- n -2
p_val <- round(2*(1-pt(test_stat,df)), 5)
```
...
::::: columns
::: {.column width="50%"}
-   `estimate`: point estimate ($b_{0}$ or $b_{1}$)

-   `std.error`: estimated standard error of the estimate $(\widehat{\text{SE}})$
:::

::: {.column width="50%"}
-   `statistic`: value of the test statistic (follows $t_{df = n-2}$ distribution for SLR)
...
::: fragment
$$
t = \frac{\text{observed} - \text{null}}{\widehat{\text{SE}_{0}}} =\frac{b_{1,obs} - \beta_{1, 0}}{\widehat{\text{SE}}_{0}} = \frac{`r round(est,3)` - 0}{`r round(se,3)`} = `r test_stat`
$$
:::
...
$$\text{Pr}(T \geq 4.09) + \text{Pr}(T \leq -4.09) \qquad \text{ where } T \sim t_{`r df`}$$
...
$$
\text{CI} = \text{point est.} \pm \text{critical value} \times \widehat{\text{SE}} \qquad \text{ (or SE if we have it)}
$$
