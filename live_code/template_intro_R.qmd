...
This document that we are working in is called an **Quarto markdown** document. It allows us to seamlessly move between `R` code and regular text. The name of the file is easily found at the top this panel. All Quarto markdown files end in ".qmd".
...
At this point it is a good idea to save our progress. Like most document editors, we need to explicitly save our work. You know your work is not saved when the file name up top is red with an asterisk.

To save our work in Quarto Markdown document, we can do one of the following:

1.  Hit Cmd+S
2.  \_\_\_\_ the document (more on this later!)
3.  Click on the floppy disk at the top of the panel
4.  Go to File, then Save
...
When we calculated $\sqrt{4}$, we used the code `sqrt()`. This is an example of a **function**. Functions allow us to automate common tasks in a general way. Functions (just like in math) take in one or more inputs. These inputs are known as \_\_\_\_\_ or \_\_\_\_\_. They will *almost* always return an output. We know a command in `R` is a function because it has parantheses.
...
Suppose I want to save the result of a big calculation to use moving forward. We can *store* or *assign* the value of 1 + 2 + 3 + 4 + 5 into the variable called `x` using the keys `<-`. Note that programming variable is not the same as statistical variables!

```{r}

```

As you should note: when we assign an object a value, its value is *not* automatically shown as output. In order to display the output, you must explicilty type the variable and run/execute that line of code.
...
Then, try installing the `tidyverse` and then the `tinytex` packages on your own! (You may see some red text; totally fine!) Once you've done that, run the following in your Console:
...
When you are finished with your work and ready to submit, you should always render one last time. You should then submit the most recent rendered output (i.e. PDF) to the corresponding Gradescope assignment. Sometimes I will also ask you to submit the accompanying .qmd file.

This document does NOT need to be submitted. It is for you!
