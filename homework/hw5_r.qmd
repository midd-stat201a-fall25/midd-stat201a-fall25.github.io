...
0. At the top of this code chunk, set a seed of 100. 

```{r}

```

1.  In code that is *as reproducible as possible*, obtain two bootstrap distributions of 5000 iterations each: one for the sample median year of pennies in circulation in 2025, and one for sample mean year. You should *not* need to run two separate for() loops. *Remember: we can access a specific column of a data frame as a vector using the `$` notation (see the coding practice as an example).*

```{r}

```

2.  For each parameter of interest, calculate and store the bounds of a 95% bootstrap confidence interval. Don't display any output here.

```{r}

```

3.  Now, let's visualize your bootstrap distribution of sample *medians* along with the corresponding confidence interval. Write code that creates a histogram of the bootstrap distribution of sample medians that includes:
...
4.  Using in-line code, interpret your two confidence intervals in context. You should include the notion of the population of interest in your interpretation.
